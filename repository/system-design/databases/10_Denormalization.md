...
- **Materialized views / read models** — Maintain a separate table or view that is a denormalized projection of normalized data; refresh periodically or via events. See [CQRS](../Patterns/2_Cqrs.md).
...
- **CQRS** — Keep writes normalized; maintain denormalized read models for queries. See [CQRS](../Patterns/2_Cqrs.md) and [Database design overview](1_Database_Design_Overview.md). Prefer **normalization** for the system of record when consistency and write simplicity matter; add denormalization where reads justify it.

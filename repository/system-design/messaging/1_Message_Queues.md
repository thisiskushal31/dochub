...
![Message queue: producers, queue, and consumers](../Assets/Messaging/Message_Queue.png)
...
**Product cases:** WhatsApp-style chat uses queues for group message fan-out ([WhatsApp case](../Cases/2_Whatsapp.md)). YouTube/Netflix use job queues for video transcoding ([YouTube / Netflix case](../Cases/5_Youtube_Netflix.md)). Uber uses queues for location ingest and analytics ([Uber case](../Cases/4_Uber.md)).
...
Use message (or task) queues to **offload slow or heavy work** (notifications, image processing, fan-out), **decouple** services, and **smooth traffic spikes**. Combine with [Task queues](2_Task_Queues.md) for job-oriented processing, [Backpressure](3_Backpressure.md) when consumers can’t keep up, and [DLQ](5_Dlq_and_Reliability.md) for failed messages.

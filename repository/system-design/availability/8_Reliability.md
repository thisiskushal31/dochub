...
- **Availability** — Percentage of time the system is up and serving. See [Availability in numbers](4_Availability_in_Numbers.md).
- **Fault tolerance** — System continues or degrades gracefully when components fail. See [Fault tolerance](7_Fault_Tolerance.md).
- **Durability** — Data is not lost on failure; achieved via replication, WAL, and backups. See [Replication](3_Replication.md), [WAL](../Storage/3_WAL_and_Durability.md).
- **Correctness** — Consistency and integrity; no silent corruption. See [Consistency](../Consistency/1_Availability_vs_Consistency.md).

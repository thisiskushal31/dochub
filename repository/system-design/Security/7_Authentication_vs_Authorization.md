...
- **API Gateway / service** — Validate token (authn), then check scope or role for the endpoint (authz). See [API Gateway](../Fundamentals/13_API_Gateway.md).
- **Federated identity** — Authn delegated to IdP (OAuth/OIDC); your app gets identity and possibly claims/roles for authz. See [Federated identity](2_Federated_Identity.md).
- **Principle of least privilege** — Grant only the permissions needed (authz); limit token scope and expiry (authn/authz). See [Security overview](1_Security_Overview.md).

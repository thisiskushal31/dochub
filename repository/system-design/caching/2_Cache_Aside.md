...
General-purpose pattern when the **application** should control what is cached and when it’s updated. Common with **Memcached** or **Redis** in front of a database. Use when you want **lazy** population and explicit invalidation; combine with TTL to avoid holding stale data forever. See [Caching overview](1_Caching_Overview.md) and [Cache layers](6_Cache_Layers.md).

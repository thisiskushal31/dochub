# Google Cloud Storage — deep dive

[← Blob/Object folder](../README.md)

*(Stub — fill this engine when you write it.)*

## Planned coverage (section summary)

- Overview & buckets
- Storage classes
- Security & IAM
- Operations

## Topic files

| # | Topic | Status |
|---|--------|--------|
| 1 | [Overview & buckets](./1_Overview_and_Concepts.md) | stub |
| 2 | [Storage classes](./2_Storage_Classes_and_Lifecycle.md) | stub |
| 3 | [Security & IAM](./3_Security_and_IAM.md) | stub |
| 4 | [Operations](./4_Operations_and_Patterns.md) | stub |

## Checklist before marking done (whole engine)

- [ ] All topic files above filled (no TBD)
- [ ] Use cases tied to [10-type table](../../README.md#database-types--use-cases)
- [ ] Operational checklist included in last topic

# 7 — Safety: LOTO and EPO

[← Previous](./6_Ticket_Taxonomy_And_Escalation.md) · [README](./README.md) · [Next: Capacity conversation →](./8_Capacity_Conversation.md)

---

## 1. Concepts

**LOTO** (lockout/tagout) isolates energy so people can work. **EPO** sheds defined loads in emergencies. Global jobs; **local code is a variant**—never import another country’s informal habits.

Deep device: [Electrical/13](../Electrical/13_EPO_And_Safety_Disconnects.md). Arc flash literacy via PPE boundaries on gear.

---

## 2. Advanced concepts

### Failure modes

| Failure | Impact |
|---------|--------|
| Incomplete energy list (battery/UPS/gen) | Re-energize surprise |
| EPO as reboot tool | Mass outage |
| Untrained operation | Injury / termination event |
| Defeat interlocks | Catastrophe |

### How it connects

Critical facilities day [3](./3_Day_In_The_Life_Critical_Facilities.md). White-space housekeeping [White-Space/10](../White-Space/10_White_Space_Safety_And_Housekeeping.md). Fire [Mechanical/8](../Mechanical/8_Fire_Detection_VESDA.md)–[9](../Mechanical/9_Fire_Suppression_Clean_Agent_And_Pre_Action.md).

### Global variants

NFPA 70E (US reference), IEC-based systems elsewhere—follow site posted rules and training.

---

## 3. Applications and use cases

| Goal | Pattern |
|------|---------|
| Any power work | Permit + LOTO + verify zero |
| New site | EPO matrix training day-1 |
| Vendor work | Confirm their LOTO integrates with yours |
| Drill | Tabletop EPO zones—not live mash |

**Staff checklist**

- Training current before touch  
- All sources listed  
- EPO authority known  
- Never joke with mushroom buttons  

**Good:** trained, permitted, verified isolation. **Bad:** cowboy breakers; EPO folklore.

---

## References

- [NFPA 70E](https://www.nfpa.org/codes-and-standards/nfpa-70e-standard-development/70e)  
- [NFPA 70](https://www.nfpa.org/codes-and-standards/nfpa-70-standard-development/70)  
- [IEC](https://www.iec.ch/)  
- [Electrical/13](../Electrical/13_EPO_And_Safety_Disconnects.md)  

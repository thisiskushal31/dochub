# 5 — Campus vs single building

[← Previous](./4_Cage_Cabinet_Suite_And_Hands.md) · [README](./README.md) · [Next: Site selection →](./6_Climate_Grid_And_Site_Selection.md)

---

## 1. Concepts

| Shape | Traits |
|-------|--------|
| **Single building** | One plant envelope; simpler ops |
| **Campus** | Multiple buildings; shared or split plant; dark fiber; phased MW |

Wholesale campuses often sell powered shells across phases ([Markets/5](../Markets-And-Operators/5_Wholesale_And_Hyperscale_Landlords.md)).

---

## 2. Advanced concepts

### Failure modes

| Mistake | Impact |
|---------|--------|
| Two buildings, one utility lateral | Correlated risk |
| Dark fiber single path | Soft SPOF |
| Calling campus “multi-AZ” without independence | Fiction |

### How it connects

Site selection [6](./6_Climate_Grid_And_Site_Selection.md). DR on-ramp [../12](../12_Sites_DR_Hybrid_And_The_Job.md).

---

## 3. Applications and use cases

| Goal | Pattern |
|------|---------|
| Scale | Campus with phased power |
| DR | True diverse buildings/metros |
| Interconnect | Prefer meet-me adjacency |

**Staff checklist**

- Utility/fiber diversity asked  
- Phase delivery dates  
- Never equate campus marketing with AZ math  

**Good:** independence proven across buildings. **Bad:** shared SPOF renamed “campus.”

---

## References

- [Uptime Institute](https://uptimeinstitute.com/)  
- [TIA-942](https://tiaonline.org/standard/tia-942/)  
- Wholesale operator campus docs (official)  
- [Markets/5](../Markets-And-Operators/5_Wholesale_And_Hyperscale_Landlords.md)  
